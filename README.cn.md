# AstrBot

[English version](./README.md)

AI Agent Assistant & development framework that integrates lots of IM platforms, LLMs, plugins and AI feature, and can be your openclaw alternative. ✨

[![x-cmd/install — AstrBot Code Quality Monitoring Repo Card](https://x-cmd.com/repo-card/AstrBot.svg?lang=zh)](https://x-cmd.com/install/AstrBot)

## 安装

```sh
x install AstrBot
```

## 代码洞察

合计: **279,077** 行代码（覆盖前 5 种语言、共 **1157** 个文件）。

| 语言 | 代码 | 注释 | 空行 | 文件数 |
|------|-----:|-----:|-----:|------:|
| Python | 197,569 | 3,649 | 30,861 | 719 |
| Json | 34,461 | 0 | 5 | 173 |
| TypeScript | 14,404 | 1,802 | 2,389 | 67 |
| Yaml | 12,798 | 57 | 2,170 | 20 |
| Vue | 12,065 | 187 | 1,034 | 178 |

## 源代码

- **上游仓库**: <https://github.com/AstrBotDevs/AstrBot>
- **官网**: <https://astrbot.app>
- **许可证**: AGPL-3.0

## 发布

- **最新版本**: `v4.28.1` (2026-09-14)
- **最近提交**: 2026-09-25
- **Release 含资产**: 1 个

## 流行度

- **Star**: 41,021 · **Fork**: 2,976 · **开放 issue**: 6,620 · **贡献者**: 385

## 累计统计

- **发布数**: 255 · **已合并 PR**: 1851 · **开放 PR**: 469 · **已关闭 issue**: 5520 · **开放 issue**: 1100 · **提交数**: 5291

## 最近活动

| 时间窗口 | 起始 | 发布 | 已合并 PR | 开放 PR | 已关闭 issue | 开放 issue | 提交 |
|---|---|---:|---:|---:|---:|---:|---:|
| 30d | 2026-08-27 | 4 | 94 | 129 | 46 | 64 | 107 |
| last60d | 2026-07-28 | 10 | 224 | 196 | 144 | 140 | 236 |
| 90d | 2026-06-28 | 15 | 335 | 241 | 242 | 194 | 372 |
| last180d | 2026-03-30 | 48 | 750 | 373 | 895 | 571 | 877 |
| 360d | 2025-10-01 | 100 | 1439 | 469 | 3241 | 1053 | 2108 |
| last720d | 2024-10-06 | 100 | 1808 | 469 | 5372 | 1100 | 4398 |

## Release 资产

| 资产 | 大小 | 目标平台 |
|------|-----:|----------|
| [AstrBot-v4.28.1-dashboard.zip](https://github.com/AstrBotDevs/AstrBot/releases/download/v4.28.1/AstrBot-v4.28.1-dashboard.zip) | 5.1 MiB | `other` |

## 改进这些数据

AstrBot 的安装元数据由 [x-cmd/install](https://github.com/x-cmd/install) 索引维护——这是一份由 x-cmd 在安装时读取的精选 YAML 包列表。如果 `AstrBot` 缺失、过期，或安装行为有问题，欢迎在该 repo 提 issue 或 PR：

- **提交 issue**: <https://github.com/x-cmd/install/issues/new>
- **编辑包条目**: <https://github.com/x-cmd/install/edit/main/AstrBot.yml>（或索引实际使用的路径）

本页面的数据（card / loc / scorecard / release）由 [x-cmd-install-action](https://github.com/x-cmd-install/x-cmd-install-action) 自动采集，每日重新生成。**安装行为**（版本选择、平台差异、依赖处理）的改进应提交到上游索引。

_数据快照: `data/card/260926.yml` · 2026-09-26T05:38:51Z._
